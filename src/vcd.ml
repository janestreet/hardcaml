...
let wrap os sim =
  ...
  let trace_in =
    List.filter trace_in ~f:(fun s ->
      not (String.equal s.name "clock") && not (String.equal s.name "reset")) in
  ...
  let write_reset () =
    ...
    List.iter trace_in ~f:(fun t ->
      write_var t.id (Bits.to_bstr !(t.data)) t.w; t.prev := (Bits.to_bstr !(t.data)));
  ...
  let write_cycle () =
    ...
    List.iter trace_in ~f:(fun t ->
      ...
      then (write_var t.id data t.w; t.prev := data));
    List.iter trace_out ~f:(fun t ->
      let data = Bits.to_bstr !(t.data) in
      if not (String.equal data !(t.prev))
      then (write_var t.id data t.w; t.prev := data));
    ...
    time := !time + vcdcycle
  in
  Private.modify sim [ After , Reset , write_reset
                     ; After , Seq   , write_cycle ]
...
module Gtkwave = struct
  ...
  let gtkwave ?(args="") sim =
    ...
    at_exit (fun () ->
      printf "Destroying FIFO\n"; Out_channel.close fifo; Unix.unlink fifoname);
    wrap fifo sim
end
