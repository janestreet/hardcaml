...
external mask : (int[@untagged]) -> Bytes.t -> unit =
  "hardcaml_bits_mask_bc" "hardcaml_bits_mask"[@@noalloc]
...
let to_int t =
  Base.Int64.to_int_trunc (to_int64 t)

let to_int32 t =
  Base.Int64.to_int32_trunc (to_int64 t)

let to_int64_array t =
  Array.init (words t) ~f:(unsafe_get t.data)
...
let of_binary_string b =
  ...
  then raise_s [%message "[Constant.of_binary_string] input string is empty"]
    ...
    let rec to_int8 b acc offset count =
      if count = 0 then Char.of_int_exn acc
      else
        let char = b.[offset] in
        if Char.(char = '0' || char = '1')
        then
          let bit = Char.to_int char - char_zero in (* 0 or 1 *)
...
let unsafe_of_bytes ~width b =
  ...
  then raise_s [%message "[Constant.of_bytes] Bytes.t length is not suitable for constant"
                           (expected_bytes : int)
                           (actual_bytes : int)
                           (width : int)];
  mask width b;
...
let of_int64_array ~width a =
  ...
  then raise_s [%message "[of_int64_array] array to large for given [width]"
                           (array_width : int) (width : int)];
...
let of_z ~width z =
  ...
  let rec f i z width =
    ...
    then ()
      ...
      let z = Z.shift_right z 32 in (* _trunc? *)
