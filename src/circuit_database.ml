...
let create () =
  ...
  List.iter (Reserved_words.(verilog @ vhdl)) ~f:(fun r ->
    ignore (Mangler.add_identifier mangler r : [ `Ok | `Duplicate ]));
  { mangler
  ; entry_by_mangled_name = Hashtbl.create (module String) }
...
let add t circuit =
  ...
  let entry =
    { Entry.
      name
    ; mangled_name
    ; circuit      = Circuit.with_name circuit ~name:mangled_name }
  in
...
let add_if_unique t circuit =
  ...
  match
    List.find (Hashtbl.data t.entry_by_mangled_name)
      ~f:(fun entry ->
...
let insert ?(share = true) database circuit =
  if share
  then add_if_unique database circuit
  else add           database circuit
