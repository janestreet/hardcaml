...
module Comms = struct
  ...
  let recv_int32 =
    ...
    (fun socket ->
       let f i =
         Int32.shift_left (Int32.of_int_exn @@ Char.to_int @@ Bytes.get b i) (i*8) in
       recv socket b 4;
       Int32.(logor
                (logor (f 0) (f 1))
                (logor (f 2) (f 3))))
...
let write_testbench ?dump_file ~name ~inputs ~outputs os =
  let declare net s =
    ...
    if width > 1
    then begin
      os "["; os (Int.to_string (width - 1)); os ":0] "
    end;
    os (fst s);
    os ";\n"
  in
  ...
  let ports =
    List.map (inputs @ outputs)
      ~f:(fun s -> "." ^ fst s ^ "(" ^ fst s ^ ")") in
...
let write_testbench_from_circuit ?dump_file os circuit =
  ...
  let outputs = List.map (Circuit.outputs circuit) ~f:(fun s -> name s, Signal.width s) in
  write_testbench ?dump_file ~name:cname ~inputs ~outputs os

let derive_clocks_and_resets circuit =
  let seq_elts =
    Signal_graph.filter (Circuit.signal_graph circuit)
      ~f:(fun s -> Signal.is_reg s || Signal.is_mem s)
  in
  ...
  let unique_names l =
    Set.to_list
      (List.fold l ~init:(Set.empty (module String)) ~f:(fun set s ->
...
module Icarus = struct
  ...
  let compile_and_load_sim ?dump_file ?opts circuit =
    ...
    at_exit (fun _ -> Unix.unlink verilog_file_name; Unix.unlink vvp_file_name);
...
module Make (SIM : Simulator) = struct
  ...
  let to_i32l x =
    let rec f x =
      ...
      then [ Bits.to_int32 x ]
        ...
        Bits.to_int32 (Bits.select x 31 0) :: f (Bits.select x (w-1) 32)
    in
    f x
  ...
  let init_sim start_sim inputs =
    ...
    in
    ignore (
      control server (Run { sets; gets = []; delta_time = 0L })
      : response_message);
    server, nets
  ...
  let make ?dump_file ?opts circuit =
    ...
    let server, nets =
      init_sim (fun () -> SIM.compile_and_load_sim ?dump_file ?opts circuit) inputs in
    ...
    in
    let clocks, resets =
      List.map clocks ~f:(fun n -> n, 1), List.map resets ~f:(fun n -> n, 1) in
    make_sim_obj ~server ~clocks ~resets ~inputs ~outputs ~nets

  let load ?opts ~clocks ~resets ~inputs ~outputs vvp_file =
    (* initialize server and simulation *)
    let server, nets =
      init_sim (fun () -> SIM.load_sim ?opts vvp_file) (clocks@resets@inputs) in
    (* create simulation object *)
    make_sim_obj ~server ~clocks ~resets ~inputs ~outputs ~nets
        ...
        (fun create_options ?(vcd_file_name = "dump.vcd") ?port_checks ?add_phantom_inputs create_fn ->
           let circuit =
             Circuit.call_with_create_options
               C.create_exn create_options ?port_checks ?add_phantom_inputs ~name:"cosim2" create_fn
           in
