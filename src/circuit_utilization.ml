...
module Total_bits = struct
  type t = { count : int; total_bits : int }
  [@@deriving sexp_of]
  let none = { count = 0; total_bits = 0 }
  let add ?(u=none) s = Some { count = u.count + 1; total_bits = u.total_bits + Signal.width s }
end
...
module Multiplexers = struct
  ...
  let none = { count = 0
             ; total_bits = 0
             ; multiplexers = Map.empty (module Int) }
  let add ?(u=none) s =
    ...
    Some { count = u.count + 1
         ...
         ; multiplexers =
             ...
             | None ->
               Map.add_exn u.multiplexers ~key
                 ~data:{ max_instance_bits = number_of_data_elements
                       ; total_bits
                       ; count = 1 }
             | Some data ->
               Map.set u.multiplexers ~key
                 ~data:{ max_instance_bits = max data.max_instance_bits total_bits
...
module Memories = struct
  ...
  let none =
    { count = 0
    ; total_bits = 0
    ; memories = Map.empty (module Memory) }
  let add ?(u=none) s =
    let data_width = Signal.width s in
    let depth =
      match s with
...
let rec create ?database circuit =
  ...
  Signal_graph.fold (Circuit.signal_graph circuit)
    ~init:{ T. name = Circuit.name circuit
          ; constants = None
          ; adders = None
          ; subtractors = None
    ...
    ~f:(fun utilization signal ->
      ...
      | Empty -> utilization
      | Const _ -> { utilization with constants = Total_bits.add ?u:utilization.constants signal }
      | Wire _ -> { utilization with wires = Total_bits.add ?u:utilization.wires signal }
      | Select _ -> { utilization with part_selects = Total_bits.add ?u:utilization.part_selects signal }
      | Reg _ -> { utilization with registers = Total_bits.add ?u:utilization.registers signal }
      | Mem _
      | Multiport_mem _ -> { utilization with memories = Memories.add ?u:utilization.memories signal }
      | Mem_read_port _ -> utilization
      | Inst _ -> { utilization with instantiations = Instantiations.add ?u:utilization.instantiations signal }
      | Op (_, op) ->
        match (op : Signal.signal_op) with
        | Signal_add -> { utilization with adders = Total_and_max_bits.add ?u:utilization.adders signal }
        | Signal_sub -> { utilization with subtractors = Total_and_max_bits.add ?u:utilization.subtractors signal }
        | Signal_mulu -> { utilization with unsigned_multipliers = Total_and_max_bits.add ?u:utilization.unsigned_multipliers signal }
        | Signal_muls -> { utilization with signed_multipliers = Total_and_max_bits.add ?u:utilization.signed_multipliers signal }
        | Signal_and -> { utilization with and_gates = Total_bits.add ?u:utilization.and_gates signal }
        | Signal_or -> { utilization with or_gates = Total_bits.add ?u:utilization.or_gates signal }
        | Signal_xor -> { utilization with xor_gates = Total_bits.add ?u:utilization.xor_gates signal }
        | Signal_not -> { utilization with not_gates = Total_bits.add ?u:utilization.not_gates signal }
        | Signal_eq -> { utilization with equals = Total_and_max_bits.add ?u:utilization.equals signal }
        | Signal_lt -> { utilization with comparators = Total_and_max_bits.add ?u:utilization.comparators signal }
        | Signal_cat -> { utilization with concatenation = Total_bits.add ?u:utilization.concatenation signal }
        | Signal_mux -> { utilization with multiplexers = Multiplexers.add ?u:utilization.multiplexers signal }
    ) |> expand_submodules
