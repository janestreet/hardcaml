...
module Native_or_derived = struct
  ...
  let unwrap = function Native t | Derived t -> t
end
...
let create ?create_fn ?create_fn_mutable () ~name ~input_widths ~output_widths =
  ...
  let (create_fn : _ Native_or_derived.t), (create_fn_mutable : _ Native_or_derived.t) =
    ...
    | None, None ->
      raise_s [%message
        "[Combinational_op.create] must be supplied either [create_fn] or [create_fn_mutable]"] in
  { name
  ; input_widths
  ; output_widths
  ; create_fn
...
let instantiate t ~inputs =
  ...
  let inputs =
    List.mapi inputs ~f:(fun i input -> "i" ^ Int.to_string i, input) in
  let outputs =
    List.mapi t.output_widths ~f:(fun i width -> "o" ^ Int.to_string i, width) in
  let instance =
