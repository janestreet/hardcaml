...
let write_testbench ?dump_file ~name ~inputs ~outputs os =
  ...
  let ports = List.map (inputs@outputs) ~f:(fun s -> "." ^ fst s ^ "(" ^ fst s ^ ")") in
...
let is_legal_char = function '1' | '0' -> true | _ -> false

let rec is_legal s i =
  try
    if is_legal_char s.[i]
    then is_legal s (i+1)
    else false
  with _ ->
    true
...
let init_sim start_sim inputs outputs =
  ...
  ignore (
    Comms.send server (Marshal.to_string (List.map (inputs@outputs) ~f:fst) [])
    : int);
  (* set all input ports to zero *)
  ignore (
    control server
      (Run { sets       = List.map inputs ~f:(fun (n, w) -> n, Bits.to_bstr (Bits.zero w))
           ; gets       = []
           ; delta_time = 0L })
    : response_message);
  server
...
let make_sim_obj ~server ~clocks ~resets ~inputs ~outputs =
  ...
  let fcycle () =
    ...
    List.iter2_exn outputs res ~f:(fun (n, v) (n', v') ->
      assert (String.equal n n'); v := Bits.const (legalise_value v'))
  in
  (* reset update *)
  let resets_1 = List.map resets ~f:(fun (n, _) -> n, "1") in
  let resets_0 = List.map resets ~f:(fun (n, _) -> n, "0") in
  let freset () =
    ignore (control server (Run { sets = resets_1; gets = []; delta_time = 10L })
            : response_message);
    ignore (control server (Run { sets = resets_0; gets = []; delta_time = 0L })
            : response_message);
  in
  (* simulation object *)
...
let make ?dump_file circuit =
  ...
  let server =
    init_sim (fun () -> compile_and_load_sim ?dump_file circuit) inputs outputs in
