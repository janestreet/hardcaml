...
let op2 op name width =
  ...
  Combinational_op.create () ~name
    ~input_widths:[ w; w ] ~output_widths:[ w ]
    ~create_fn
...
let op1 op name width =
  let create_fn = function
    ...
    | _ -> raise_s [%message
             "Float operation requires 1 argument" (name : string)]
  in
  let w = Width.num_bits width in
  Combinational_op.create () ~name
    ~input_widths:[ w ] ~output_widths:[ w ]
    ~create_fn

module F (P : sig val width : Width.t end) = struct
