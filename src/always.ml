...
struct
  ...
  module Variable_ = struct
    ...
    let wire ?(here = Stdlib.Lexing.dummy_pos) ~default () =
      wire' ~default (User_created here)
    ;;
  ...
  let if_ ?(here = Stdlib.Lexing.dummy_pos) sel on_true on_false =
    if_' sel on_true on_false { kind = Condition If; creation_pos = here }
  ;;

  let elif ?(here = Stdlib.Lexing.dummy_pos) c t f =
  ...
  let proc ?(here = Stdlib.Lexing.dummy_pos) s =
  ...
  module State_machine = struct
    ...
    let create
      ...
      let switch ?(here = Stdlib.Lexing.dummy_pos) ?default cases =
        let cases =
