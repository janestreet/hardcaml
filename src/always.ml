...
module Variable = struct
  ...
  let sexp_of_t { value; internal = _ } =
    [%message "" ~_:(value : Signal.t)]
  ;;
  ...
  let pipeline ~depth (spec : Reg_spec.t) ~e:enable ~width =
    ...
    then (
      ...
      if Signal.is_empty spec.reg_reset_value
...
let if_ sel on_true on_false =
  If (sel, on_true, on_false)
    ...
    let switch ?default cases =
      let rec unique set = function
        ...
        | (state, _) :: tl ->
          ...
          unique (Set.add set state) tl in
      ...
      let cases =
        ...
        | None ->
          ...
          cases
        | Some default ->
          cases @ (Set.to_list unhandled_states
                   |> List.map ~f:(fun state -> state, default))
      in
      match encoding with
      | Binary | Gray ->
      ...
      | Onehot ->
        proc (
          List.map cases ~f:(fun (s, c) ->
            ...
            when_ (Signal.bit current i) c)) in
    let is s =
      match encoding with
      | Binary | Gray -> state_val "is" s ==: current
      | Onehot -> Signal.bit var.value (fst (find_state "is" s)) in
