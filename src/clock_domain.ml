...
module Exact = struct
  ...
  let create_domain ?(loc = Stdlib.Lexing.dummy_pos) name =
    { uid = gen_uid (); name; loc }
  ;;
...
let create_with_uid ?(loc = Stdlib.Lexing.dummy_pos) name uid = { uid; name; loc }
let create ?(loc = Stdlib.Lexing.dummy_pos) name = create_with_uid ~loc name (gen_uid ())
let get_name ({ name; _ } : t) = name

let generate_fresh_exact_domain ?(loc = Stdlib.Lexing.dummy_pos) (t : _) =
