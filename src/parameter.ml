...
module Std_logic_vector = struct
  ...
  let to_string v =
    v
    |> List.map ~f:Std_logic.to_char
    |> String.of_char_list

  let of_string s =
    s
    |> String.to_list
    |> List.map ~f:Std_logic.of_char_exn
...
module Bit_vector = struct
  ...
  let to_string v =
    v
    |> List.map ~f:(function true -> '1' | false -> '0')
    |> String.of_char_list
...
let find_name ts name =
  List.find_map ts ~f:(fun t ->
    if Parameter_name.equal t.name name
    then Some t.value
    else None)
