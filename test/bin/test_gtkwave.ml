...
let test_gtkwave () =
  ...
  S.reset sim;
  i.enable := Bits.vdd;
  i.d :=  Bits.consti ~width:16 1;    S.cycle sim;
  i.d :=  Bits.consti ~width:16 3;    S.cycle sim;
  i.d :=  Bits.consti ~width:16 (-2); S.cycle sim;
  i.d :=  Bits.consti ~width:16 4;    S.cycle sim; S.cycle sim;
