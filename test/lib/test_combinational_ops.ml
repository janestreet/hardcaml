...
let%expect_test "no inputs" =
  require_does_not_raise [%here] (fun () ->
    Combinational_op.create ()
      ~name:"no_inputs"
      ~input_widths:[] ~output_widths:[1]
      ~create_fn:(fun _ -> []) |> ignore);
  [%expect {| |}]
...
let%expect_test "bad output width" =
  require_does_raise [%here] (fun () ->
    Combinational_op.create ()
      ~name:"bad_width"
      ~input_widths:[1] ~output_widths:[0]
      ~create_fn:(fun _ -> []));
...
let%expect_test "bad input width" =
  require_does_raise [%here] (fun () ->
    Combinational_op.create ()
      ~name:"bad_width"
      ~input_widths:[0] ~output_widths:[1]
      ~create_fn:(fun _ -> []));
...
let%expect_test "names must be unique in database" =
  let op =
    Combinational_op.create ()
      ~name:"foo"
      ~input_widths:[1] ~output_widths:[1]
      ~create_fn:(fun a -> a)
  in
...
let create_op () =
  Combinational_op.create ()
    ~name:"add_sub"
    ~input_widths:[num_bits; num_bits] ~output_widths:[num_bits; num_bits]
    ~create_fn:(function [ a; b ] -> Bits.([ a +: b; a -: b ])
...
let testbench ~create_circuit ~create_sim =
  ...
  List.init 4 ~f:(fun i ->
    List.init 4 ~f:(fun j ->
      a := Bits.consti num_bits i;
      b := Bits.consti num_bits j;
      Cyclesim.cycle sim;
      !a, !b, !c, !d))
  |> List.concat
...
let%expect_test "functional sim / bits" =
  ...
  let sexp_of_result (a, b, c, d) =
    [%message "" (a : Bits.t) (b : Bits.t) (c : Bits.t) (d : Bits.t)] in
