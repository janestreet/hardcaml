...
let sim combinational_ops_database (op1:op1 list) (op2:op2 list) args =
  ...
  let f (i : Signal.t I.t) =
    List.mapi ops ~f:(fun j op ->
      { O.
        o32 = output ("o32_" ^ Int.to_string j) (op.hwop32 i.a32 i.b32)
      ; o64 = output ("o64_" ^ Int.to_string j) (op.hwop64 i.a64 i.b64) })
  in
  (* simulator *)
  let module Cs = Cyclesim in
  let i = I.map ~f:(fun (n, b) -> input n b) I.t in
  let circuit = Circuit.create_exn ~name:"test" (List.concat (List.map (f i) ~f:O.to_list)) in
  let sim = Cs.create ~kind:Immutable ~combinational_ops_database circuit in
  (* simulator ports *)
  let i : Bits.t ref I.t = I.map ~f:(fun (n, _) -> S.in_port sim n) I.t in
  let o : Bits.t ref O.t list =
    List.mapi ops
      ~f:(fun j _ -> { O.
                       o32 = S.out_port sim ("o32_" ^ Int.to_string j)
  ...
  let step (arg_a, arg_b) =
    ...
    let result_in_error_bounds args =
      ...
      if      Float.(abs (expected - got_32bit) > 1e-6) then Error result
      else if Float.(abs (expected - got_64bit) >  0.0) then Error result
      else Ok opname
    in
...
let%expect_test "floating point operations in simulation" =
  ...
  let op1s : op1 list =
    ...
    ; { name = "log10"; fop = Float.log10; hwop32 = Fops.Float.log10; hwop64 = Fops.Double.log10 }
    ...
    ; { name = "acos" ; fop = Float.acos ; hwop32 = Fops.Float.acos ; hwop64 = Fops.Double.acos  }
    ; { name = "asin" ; fop = Float.asin ; hwop32 = Fops.Float.asin ; hwop64 = Fops.Double.asin  }
    ; { name = "atan" ; fop = Float.atan ; hwop32 = Fops.Float.atan ; hwop64 = Fops.Double.atan  }
    ; { name = "cosh" ; fop = Float.cosh ; hwop32 = Fops.Float.cosh ; hwop64 = Fops.Double.cosh  }
    ; { name = "sinh" ; fop = Float.sinh ; hwop32 = Fops.Float.sinh ; hwop64 = Fops.Double.sinh  }
    ; { name = "tanh" ; fop = Float.tanh ; hwop32 = Fops.Float.tanh ; hwop64 = Fops.Double.tanh  }
    ; { name = "ceil" ; fop = Caml. ceil ; hwop32 = Fops.Float.ceil ; hwop64 = Fops.Double.ceil  }
    ; { name = "floor"; fop = Caml. floor; hwop32 = Fops.Float.floor; hwop64 = Fops.Double.floor }
    ; { name = "abs"  ; fop = Float.abs  ; hwop32 = Fops.Float.abs  ; hwop64 = Fops.Double.abs   }]
  in
  let op2s : op2 list =
    [ { name = "+"    ; fop = Float.( + )    ; hwop32 = Fops.Float.( +: ) ; hwop64 = Fops.Double.( +: )  }
    ; { name = "-"    ; fop = Float.( - )    ; hwop32 = Fops.Float.( -: ) ; hwop64 = Fops.Double.( -: )  }
    ; { name = "*"    ; fop = Float.( * )    ; hwop32 = Fops.Float.( *: ) ; hwop64 = Fops.Double.( *: )  }
    ; { name = "/"    ; fop = Float.( / )    ; hwop32 = Fops.Float.( /: ) ; hwop64 = Fops.Double.( /: )  }
    ; { name = "%"    ; fop = Float.mod_float; hwop32 = Fops.Float.( %: ) ; hwop64 = Fops.Double.( %: )  }
    ; { name = "**"   ; fop =       ( **. )  ; hwop32 = Fops.Float.( **: ); hwop64 = Fops.Double.( **: ) }
    ; { name = "atan2"; fop = Float.atan2    ; hwop32 = Fops.Float.atan2  ; hwop64 = Fops.Double.atan2   }]
  in
  let database = Combinational_ops_database.concat [ Fops.Float.database
                                                   ; Fops.Double.database ] in
  print_s [%sexp (sim database op1s op2s
                    [ 0., 0.
                    ; 1.5, 2.3
                    ; -0.7, 3.4 ] : all_op_results list) ];
  [%expect {|
    (((arg_a 0)
      (arg_b 0)
      (results (
