...
let abits_int_of_bstr, bstr_of_abits_int =
  convert_abits_and_string (module XInt)
...
module T = struct
  ...
  let to_int (data, width) =
    ...
    if width <= nbits
    then data.(0)
    else data.(0) lor (data.(1) lsl nbits)
  ...
  let to_constant (b,width) =
    bstr_of_abits_int ~width b  |> Constant.of_binary_string
  ...
  let (<:) a b =
    ...
    let set_cmp a b =
      if !cmp = 0
      then (
        if Int.compare a b > 0
        then cmp := 1
        else if Int.compare a b < 0
        then cmp := -1)
    in
    iterback
      (fun a b ->
  ...
  let umul n a b =
    ...
    let c, _ =
      List.fold sums ~init:(gnd, 0) ~f:(fun (acc, scl) e ->
        acc +: scale e scl, scl + n)
    in
...
module Mutable = struct
  ...
  module T = struct
    ...
    let (==:) c a b =
      ...
      iterback (fun a b -> if not (a = b) then eq := false) a b;
      if !eq
      then copy ~dst:c ~src:vdd
      else copy ~dst:c ~src:gnd
    ...
    let concat c l =
      ...
      then ()
        ...
        ignore (List.fold (List.tl_exn l) ~init:c ~f:(fun c a -> cat c a))
  ...
  module Comb =
    Comb.Make (struct
      ...
      let to_string = T.to_string
      let sexp_of_t s = [%sexp (to_constant s
                                |> Constant.to_binary_string : string)]
    end)
end
