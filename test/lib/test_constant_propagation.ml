...
module Trace = struct
  ...
  let sexp_of_fn1 sexp_of_a (a, r) =
    [%message "" ~_:(a : a) ~_:"=" ~_:(r : signal)]
  ...
  let%expect_test "test trace" =
    ...
    print_s [%message "~:" ~_:(fn1 (~:) (const "10") : signal fn1)];
  ...
  let op2 bits ~f =
    ...
    List.init (1 lsl bits) ~f:(fun x ->
      List.init (1 lsl bits) ~f:(fun y ->
        f (consti bits x) (consti bits y)))

  let op1 bits ~f =
    List.init (1 lsl bits) ~f:(fun y -> f (consti bits y))
  ...
  let binary_op_tests name (+:) (+:.) =
    ...
    [%message
      name
        ...
        ~misc:         ([ consti 8 22 +: consti 8 33
                        ; consti 123 22345 +: consti 123 (-22345) ] : signal op2 list)
        ~int_on_right: ([ consti 7 27 +:. 12
                        ; consti 7 27 +:. (-12) ] : int op2 list) ]
  ...
  let mul_op_tests name ( *: ) =
    ...
    [%message
      name
        ~all_1_bit:  (op2 1 ~f:( *: ) : signal op2 list)
        ~all_2_bits: (op2 2 ~f:( *: ) : signal op2 list)
        ~pow2:       ([ consti 2 3 *: consti 5 1
                      ; consti 2 3 *: consti 5 2
                      ; consti 2 3 *: consti 5 4
                      ; consti 2 3 *: consti 5 8
                      ; consti 2 3 *: consti 5 16] : signal op2 list)
        ~misc:       ([ consti 8 22 *: consti 8 33
                      ; consti 7 27 *: consti 4 12 ] : signal op2 list) ]
end
...
let%expect_test "concat" =
  print_s [%message
    ""
      ~concat:(List.map ~f:(fn1 concat)
                 [ [ vdd; gnd ]
                 ; [ vdd; gnd; vdd ]
                 ; [ one 4; ones 3; zero 2 ] ] : signal list fn1 list)
      ~concat_e:(List.map ~f:(fn1 concat_e)
                   [ [ vdd; empty ]
                   ; [ empty; vdd ]
                   ; [ empty; vdd; empty; gnd ]
                   ; [ consti 3 2; empty; consti 2 1 ] ] : signal list fn1 list)
...
let%expect_test "repeat" =
  ...
  print_s [%message
    "repeat"
      ~_:([ repeat vdd 3
          ; repeat (const "01") 4
          ; repeat (consti 8 123) 1 ] : (signal, int) fn2 list) ];
  [%expect {|
    (repeat (
      ((1'b1        3) = 3'b111)
      ((2'b01       4) = 8'b01010101)
...
let%expect_test "reduce" =
  let d =
    ...
    ; [ vdd; gnd; gnd; vdd ] ]
  in
  let reduce op = fn1 (reduce op) in
  print_s [%message
    "reduce"
...
let%expect_test "mod_counter" =
  let mod_counter n = fn1 (mod_counter n) in
  print_s [%message
    "mod_counter"
      ~mod_4:(List.init 8 ~f:(fun i -> mod_counter 3 (consti 3 i)) : signal fn1 list)
      ~mod_7:(List.init 8 ~f:(fun i -> mod_counter 6 (consti 3 i)) : signal fn1 list) ];
  [%expect {|
    (mod_counter
      (mod_4 (
        (3'b000 = 3'b001)
...
let%expect_test "tree" =
  let tree n op = fn1 (tree n op) in
  print_s [%message
    "tree with different branching factors"
      ~add_branch_2:(tree 2 (reduce (+:))
                       (List.map ~f:(consti 10) [ 10; 20; 30; 40; 50; 60; 70 ])
                     : signal list fn1)
      ~add_branch_3:(tree 3 (reduce (+:))
                       (List.map ~f:(consti 10) [ 10; 20; 30; 40; 50; 60; 70 ])
                     : signal list fn1 ) ];
  [%expect {|
    ("tree with different branching factors"
...
let%expect_test "binary_to_onehot" =
  ...
  print_s [%message
    "binary_to_onehot"
      ~_:(List.init 4 ~f:(fun i -> binary_to_onehot (consti 2 i))
          : signal fn1 list) ];
  [%expect {|
    (binary_to_onehot (
      (2'b00 = 4'b0001)
...
let%expect_test "onehot_to_binary" =
  ...
  print_s [%message
    "onehot_to_binary"
      ~_:(List.init 4 ~f:(fun i -> onehot_to_binary (consti 4 (1 lsl i)))
          : signal fn1 list) ];
  [%expect {|
    (onehot_to_binary (
      (4'b0001 = 2'b00)
...
let%expect_test "gray_to_binary" =
  ...
  print_s [%message
    "gray_to_binary"
      ~_:(List.init 8 ~f:(fun i -> gray_to_binary (consti 3 i)) : signal fn1 list) ];
  [%expect {|
    (gray_to_binary (
      (3'b000 = 3'b000)
      (3'b001 = 3'b001)
...
let%expect_test "binary_to_gray" =
  ...
  print_s [%message
    "binart_to_gray"
      ~_:(List.init 8 ~f:(fun i -> binary_to_gray (consti 3 i)) : signal fn1 list) ];
  [%expect {|
    (binart_to_gray (
      (3'b000 = 3'b000)
      (3'b001 = 3'b001)
  ...
  iter_all_inputs ~min_width:2 ~max_width:5 ~f:(fun input ->
    ...
    require [%here]
      ...
      ~if_false_then_print_s:(
        lazy [%message (input : signal) (output : signal)]));
  print_s [%sexp (!num_tests : int)];
  [%expect {| 60 |}]

let%expect_test "[uresize 0]" =
  require_does_raise [%here] (fun () -> uresize (constb "0") 0);
  [%expect {|
    ("[select] got [hi < lo]"
      (hi -1)
      (lo 0)) |}];
...
let%expect_test "[sresize 0]" =
  require_does_raise [%here] (fun () -> sresize (constb "0") 0);
  [%expect {|
    ("[select] got [hi < lo]"
      (hi -1)
      (lo 0)) |}];
...
let%expect_test "resizing" =
  ...
  print_s [%message
    ""
      ~uresize_0:   (List.init 3 ~f:(fun i -> uresize (constb   "0") (i+1))
                     : (signal, int) fn2 list)
      ~uresize_1:   (List.init 3 ~f:(fun i -> uresize (constb   "1") (i+1))
                     : (signal, int) fn2 list)
      ~uresize_100: (List.init 5 ~f:(fun i -> uresize (constb "100") (i+1))
                     : (signal, int) fn2 list)
      ~sresize_0:   (List.init 3 ~f:(fun i -> sresize (constb   "0") (i+1))
                     : (signal, int) fn2 list)
      ~sresize_1:   (List.init 3 ~f:(fun i -> sresize (constb   "1") (i+1))
                     : (signal, int) fn2 list)
      ~sresize_100: (List.init 5 ~f:(fun i -> sresize (constb "100") (i+1))
                     : (signal, int) fn2 list)
...
let%expect_test "[select]" =
  ...
  iter_all_inputs ~min_width:2 ~max_width:5 ~f:(fun input ->
    ...
    for split = 0 to width - 2 do
      incr num_tests;
      let output =
        concat_e [ select input (width - 1) (split + 1)
                 ; select input split 0 ] in
      require [%here] (Sexp.equal [%sexp (input : signal)] [%sexp (output : signal)])
        ~if_false_then_print_s:(
...
let%expect_test "select" =
  print_s [%message
    "select"
      ~_0_up:            (List.init 4 ~f:(fun i -> fn3 select (const "1100") i    0 )
                          : (signal, int, int) fn3 list)
      ~_3_down:          (List.init 4 ~f:(fun i -> fn3 select (const "1100") 3 (3-i))
                          : (signal, int, int) fn3 list)
      ~middle:           (fn3 select (const "0110") 2 1 : (signal, int, int) fn3)
      ~_64_bit_boundary: (fn3 select (consthu 68 "18000000000000000") 65 62
                          : (signal, int, int) fn3)
      ~bit:              (List.init 4 ~f:(fun i -> fn2  bit (const "1100") i)
                          : (signal, int) fn2 list)
      ~lsbs:             (List.init 4 ~f:(fun i -> fn1 lsbs (one (i+2)))
                          : signal fn1 list)
      ~msbs:             (List.init 4 ~f:(fun i -> fn1 msbs (one (i+2)))
                          : signal fn1 list)
      ~lsb:              (List.init 4 ~f:(fun i -> fn1 lsb  (one (i+1)))
                          : signal fn1 list)
      ~msb:              (List.init 4 ~f:(fun i -> fn1 msb  (one (i+1)))
                          : signal fn1 list)
      ~drop_bottom:      (List.init 4 ~f:(fun i -> fn2 drop_bottom (const "11100") (i+1))
                          : (signal, int) fn2 list)
      ~drop_top:         (List.init 4 ~f:(fun i -> fn2 drop_top (const "11100") (i+1))
                          : (signal, int) fn2 list)
      ~sel_bottom:       (List.init 5 ~f:(fun i -> fn2 sel_bottom (const "11100") (i+1))
                          : (signal, int) fn2 list)
      ~sel_top:          (List.init 5 ~f:(fun i -> fn2 sel_top (const "11100") (i+1))
                          : (signal, int) fn2 list)
...
let%expect_test "insert" =
  print_s [%message
    "insert"
      ~insert_010_into_0s:(List.init 6 ~f:(insert ~t:(zero 8) ~f:(const "010"))
                           : signal list)
      ~insert_010_into_1s:(List.init 6 ~f:(insert ~t:(ones 8) ~f:(const "010"))
                           : signal list) ];
  [%expect {|
    (insert
      (insert_010_into_0s (
...
let%expect_test "split_in_half" =
  ...
  let sexp_of_left_and_right (left, right) = [%message "" (left:signal) (right:signal) ] in
  print_s [%message
    "split"
      ~_:(List.init 8 ~f:(fun b -> split_in_half (create (b+2))) : left_and_right list) ];
  [%expect {|
    (split (
...
let%expect_test "split" =
  let split ?exact ~part_width t =
    print_s [%sexp (split ?exact ~part_width t : signal list)] in
  let split_raises ~part_width t =
    require_does_raise [%here]
      (fun () -> split ~part_width t) in
  ...
  split ~part_width:1 (consti 2 1);
  [%expect {| (1'b1 1'b0) |}];
  split ~part_width:2 (consti 2 1);
  [%expect {| (2'b01) |}];
  split ~part_width:4 (consti 16 0x4321);
  [%expect {| (4'b0001 4'b0010 4'b0011 4'b0100) |}];
  split_raises ~part_width:4 (consti 15 0x4321);
  [%expect {|
    ("[split ~exact:true] unable to split exactly"
     (input_width        15)
     (part_width         4)
     (width_of_last_part 3)) |}];
  split ~exact:false ~part_width:4 (consti 15 0x4321);
...
let%expect_test "shifting" =
  print_s [%message
    "shifting"
      ~sll:  (List.init 4 ~f:(fn2 sll  (const "001"))
              : (signal, int) fn2 list)
      ~srl:  (List.init 4 ~f:(fn2 srl  (const "100"))
              : (signal, int) fn2 list)
      ~sra:  (List.init 4 ~f:(fn2 sra  (const "100"))
              : (signal, int) fn2 list)
      ~rotl: (List.init 4 ~f:(fn2 rotl (const "001"))
              : (signal, int) fn2 list)
      ~log_shift_sll:  (List.init 4 ~f:(fun i -> fn2 (log_shift sll)  (const "001") (consti 2 i))
                        : (signal, signal) fn2 list)
      ~log_shift_srl:  (List.init 4 ~f:(fun i -> fn2 (log_shift srl)  (const "100") (consti 2 i))
                        : (signal, signal) fn2 list)
      ~log_shift_sra:  (List.init 4 ~f:(fun i -> fn2 (log_shift sra)  (const "100") (consti 2 i))
                        : (signal, signal) fn2 list)
      ~log_shift_rotl: (List.init 4 ~f:(fun i -> fn2 (log_shift rotl) (const "001") (consti 2 i))
                        : (signal, signal) fn2 list) ];
  [%expect {|
    (shifting
      (sll (
...
let%expect_test "add width exn" = require_does_raise [%here] (fun () -> consti 3 22 +: consti 8 33);
  [%expect {|
...
let%expect_test "sub width exn" =
  require_does_raise [%here] (fun () -> consti 3 22 -: consti 8 33);
  [%expect {|
    ("[-:] got inputs of different widths" (
      (const (width 3) (value 0b110))
      (const (width 8) (value 0b00100001)))) |}]
...
let%expect_test "select hi<lo throws exn" =
  require_does_raise [%here] (fun () -> select (consti 2 0) 0 1);
  [%expect {|
    ("[select] got [hi < lo]"
      (hi 0)
      (lo 1)) |}]
