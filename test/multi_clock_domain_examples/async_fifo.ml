...
let%expect_test "show expected behavior of the async fifo" =
  let waves, { Fifo_sim.simulator; _ } =
    Fifo_sim.with_waveterm ~config:Config.trace_all (fifo ()) (fun inputs _outputs ->
  ...
  Waveform.expect waves ~wave_width:(-1) ~display_width:86 ~display_height:54;
  [%expect
    {|
    ┌Signals───────────┐┌Waves───────────────────────────────────────────────────────────┐
    │almost_empty      ││────────────────────────────────────────────────────────────────│
