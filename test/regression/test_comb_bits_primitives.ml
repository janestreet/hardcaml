...
let command =
  Command.basic
    ...
    (let open Command.Let_syntax in
     let%map_open () = return ()
     and bits =
       flag "-bits" (required Bits_module.arg_type)
         ~doc:("MODULE bits implementation to test: "
               ^ String.concat ~sep:", "
     ...
     and iterations =
       flag "-iterations" (optional_with_default 10 int)
     ...
     and prims =
       flag "-primitives"
         (optional_with_default Prim.all (Arg_type.comma_separated Prim.arg_type ~allow_empty:true))
         ~doc:("PRIM,... primitives to test: "
               ^ String.concat ~sep:", " (List.map Prim.all ~f:Prim.name))
     and verbose = flag "-verbose" no_arg ~doc:" print every test"
     in
     fun () ->
       let module T =
       ...
       in
       T.test ~stop_on_first_primitive_error:first_error_only [%here]
         { bits1         = bits_ref
         ; bits2         = bits
         ; prims
         ; iterations
