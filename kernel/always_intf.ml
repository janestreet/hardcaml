...
module type S = sig
  ...
  module Variable : sig
    ...
    (** create a wire *)
    val wire : ?here:Stdlib.Lexing.position -> default:Signal.t -> unit -> t

    (** create a register *)
    val reg : ?here:Stdlib.Lexing.position -> (width:int -> t) Signal.with_register_spec
  ...
  (** if statement *)
  val if_ : ?here:Stdlib.Lexing.position -> Signal.t -> t list -> t list -> t

  (** else if branch *)
  val elif : ?here:Stdlib.Lexing.position -> Signal.t -> t list -> t list -> t list

  (** else branch (for readability) *)
  val else_ : t list -> t list

  (** if sel then [...] else [] *)
  val when_ : ?here:Stdlib.Lexing.position -> Signal.t -> t list -> t

  (** if sel then [] else [...] *)
  val unless : ?here:Stdlib.Lexing.position -> Signal.t -> t list -> t

  (** switch statement *)
  val switch : ?here:Stdlib.Lexing.position -> Signal.t -> Signal.t cases -> t

  (** Allows sequences of expressions to be inserted into the code; a syntactic nicety. *)
  val proc : ?here:Stdlib.Lexing.position -> t list -> t
  ...
  module State_machine : sig
    type 'a t =
      ...
      ; switch :
          ?here:Stdlib.Lexing.position -> ?default:always list -> 'a cases -> always
      }
    [@@deriving sexp_of]
