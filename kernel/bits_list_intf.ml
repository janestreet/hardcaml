...
module type Comb_binable = sig
  type t [@@deriving bin_io]

  include Comb with type t := t
end

module type Bits_list = sig
  module type T = T
  module type Comb = Comb
  module type Comb_binable = Comb_binable

  (** From [T] construct the combinational API *)
  module Make (T : T) : Comb with type t = T.t list

  module Make_binable (T : T_binable) : Comb_binable with type t = T.t list

  (** Uses [0,1] as the bit type. *)
  module Int_comb : Comb_binable with type t = int list

  (** Uses [false,true] as the bit type. *)
  module Bool_comb : Comb_binable with type t = bool list
  ...
  module X_comb : Comb_binable with type t = X.t list
end
